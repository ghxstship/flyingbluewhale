...
- **Design SSOT**: the attached **kit v6.0** (`design_handoff_atlvs_kit`) is the single source of truth. CSS token SSOT is `src/app/theme/themes/atlvs-product.css` (the `--p-*` namespace); the machine-readable export is vendored at **`src/app/theme/tokens.json`** (v6.0 "Monument"). v6.0 adds the cross-app **Documents system** (`kit-documents.css` format layer + `data-path` merge contract + `data-brand` white-label modes); tokens are unchanged from v5.1. The retired cosmic GHXSTSHIP token export (Big Shoulders / Space Grotesk / Silkscreen / void-ink / voyage register / RRR project IDs) and the XPMS dual-nav + `/ghxstship` cosmic route are deleted — do not reintroduce.
