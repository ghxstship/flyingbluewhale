...
- **Vercel:** `vercel.json` sets security headers + API no-cache. Add `atlvs.pro`, `www.atlvs.pro`, `app.atlvs.pro`, `gvteway.atlvs.pro`, `compvss.atlvs.pro` as project domains (all four point to the same deployment; `src/proxy.ts` does the host rewrite).
