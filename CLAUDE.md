...
- **Sub-products (v5 roles)** — four apps, one record store, one design system:
  ...
  - **GVTEWAY** (cyan `#12B5B5`, the `/p` portal + public marketplace) — _Public Interface & Marketplace._ Every public/engagement surface: tickets, stores, directory, jobs, peer-to-peer, RFPs, + the host & commerce console.
  - **LEG3ND** (Production Orange `#E8500A`, the `/console/legend/*` Knowledge surface) — _Knowledge · LMS · Resources_ on the **XPMS 2.0 protocol_**: The Standard (knowledge base), Courses & LMS, Certifications, Resources hub, Catalog (priced atoms/URIDs), **Signage library** (ISO 7010 / DOT-AIGA / ISA pictograms — `public/brand/pictograms.svg`), **XMCE compliance engine**, Safety. Wordmark `L E G 3 N D` (the `3` in accent).
...
- **Design SSOT**: the attached **kit v5.0** (`design_handoff_atlvs_kit`) is the single source of truth. CSS token SSOT is `src/app/theme/themes/atlvs-product.css` (the `--p-*` namespace); the machine-readable export is vendored at **`src/app/theme/tokens.json`** (v5.0 "Monument"). The retired cosmic GHXSTSHIP token export (Big Shoulders / Space Grotesk / Silkscreen / void-ink / voyage register / RRR project IDs) and the XPMS dual-nav + `/ghxstship` cosmic route are deleted — do not reintroduce.
