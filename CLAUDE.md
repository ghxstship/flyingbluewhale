...
- **Sub-products (v4 roles)** — three apps, one record store, one design system:
...
- **Design SSOT**: the attached **kit v4.0** (`design_handoff_atlvs_kit`) is the single source of truth. CSS token SSOT is `src/app/theme/themes/atlvs-product.css` (the `--p-*` namespace); the machine-readable export is vendored at **`src/app/theme/tokens.json`** (v4.0 "Monument"). The retired cosmic GHXSTSHIP token export (Big Shoulders / Space Grotesk / Silkscreen / void-ink / voyage register / RRR project IDs) and the XPMS dual-nav + `/ghxstship` cosmic route are deleted — do not reintroduce.
