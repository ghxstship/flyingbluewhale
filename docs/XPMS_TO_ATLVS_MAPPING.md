...
| ---------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- |
| **Canonical XPMS production lifecycle (8-gate)**            | Discovery → Design → Advance → Procurement → Build → Install → Operate → Close                       | `src/lib/xpms` (`XPMS_PHASES`)                                                                                                                                       | **Canonical — the single 8-phase arc**    |
| **`xpms_phase` enum (project macro-phase)**                | Discovery → Design → Advance → Procurement → Build → Install → Operate → Close                       | [src/lib/xpms/index.ts](src/lib/xpms/index.ts) · `projects.xpms_phase` · `budgets.xpms_phase`                                                                        | **Implementation — what runs in prod**    |
| **`xpms_atom_phase` enum (atom-level axis)**               | discovery → design → advance → procurement → build → install → operate → close                       | `XPMS_ATOM_PHASES` · `xpms_atoms.phase`                                                                                                                              | Implementation — reconciled to canonical  |
| **`production_phase` enum (fabrication)**                  | DISCOVERY → DESIGN → ADVANCE → PROCUREMENT → BUILD → INSTALL → OPERATE → CLOSE                       | `src/lib/production-phase.ts` · `fabrication_orders.production_phase`                                                                                                 | Implementation — reconciled to canonical  |

**Resolution (canonical, reconciled 2026-06-15):**

- There is **one** 8-phase production lifecycle: **Discovery → Design → Advance → Procurement → Build → Install → Operate → Close**.
- All three phase enums now carry this single sequence (case per enum convention): `xpms_phase` (project macro), `xpms_atom_phase` (atom-level), and `production_phase` (fabrication) were reconciled in migration `20260615163006_xpms_phase_canon_reconcile_atom_production`. The earlier divergent vocabularies (concept/development/show/wrap; CONCEPT/ENGINEERING/PRE_PRO/FAB/LOGISTICS/ARCHIVED) are retired.
- Display copy (marketing lifecycle, fabrication UI, atom rollups) derives from these enums / `XPMS_PHASES`, so the phase copy cannot drift from the protocol.
...
| -------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `phase`  | Time-bounded macro arc, sequential, gated                                             | `xpms_phase`, `xpms_atom_phase`, `production_phase`          | Complete — all three carry the canonical 8-gate arc (Discovery → … → Close); reconciled in migration `20260615163006`                                                                                            |
