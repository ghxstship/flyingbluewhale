...
| Lost & Found tri-state | Kit-29 spec names Held / Claimed / **Donated**; the store (`incidents`, `report_kind='lost_property'`) supports two honest states (open = Held, closed = Claimed). Donated needs a store facet (a claim-outcome column or a sanctioned lens rule) — kit decision before the repo paints a third chip. |
| CoC sealed/unsealed | Kit-29 spec names sealed/unsealed evidence states; `assignments`/`assignment_events` carry no seal facet. Needs a store decision (Olympic-scope evidence handling) before the timeline can render it. |
| Invite FormScreen | Kit-29 spec says /m/settings/team/invite is the `invite` FormScreen — but the kit's own `invite` FormDef ships prototype seed options (target: "GHXSTSHIP", "Miami Music Week", …). The live native form with real org/project options stays until the kit def takes live options. |
