...
## Deep re-audit vs the on-disk prototype (2026-07-20)

An independent surface-by-surface diff of the repo against the canonical prototype (`templates/_app-runtime/compvss-field/{app,forms,recorddetail,tools}.jsx`). Headline: that on-disk prototype is a **pre-kit-31 generation** (tab-5 = Assets, no Aurora, no construction facets, no crisis layer, status-enum mocks instead of real tables) — the repo has deliberately, ratified-ly evolved past it. Kit-34's "100% conformant" holds; one real wiring defect surfaced, plus four gaps the product owner chose to close.

- **Assets eyebrow (defect, fixed):** `AssetsView` declared `eyebrow?` but never rendered it — Assets was the only list surface showing no context line while every peer (Inbox/Tasks/Schedule) renders one. Wired through the canonical `.backbtn → .scr-eye → .scr-h` header. (The prototype's decorative *kicker* stays suppressed per **B5**; this restores the data-context `.scr-eye`, which is the live convention.)
- **#1 Break punch (built):** the live clock face gained Start/End Break, an "On Break" state, and break toasts. Migration `20260720194758_time_entries_break_punch` adds `break_open_at` + `break_minutes` to `time_entries` (net worked = duration − break); the `/api/v1/time/clock` endpoint gained `break_start`/`break_end` (idempotent under offline replay; clock-out folds an open break). Break state is distinct from the roster `shifts.attendance='on_break'`.
- **#2 Clock-out → handover (built):** a completed **online** clock-out presents the existing `/m/handover/new` form (dismissible; offline clock-out stays on the face so the sync chip isn't buried).
- **#3 Rose "Access & Permissions" (built, honest):** replaced the prototype's hardcoded `PASS_PERMS` fiction (Petty Cash $250/day, Pyro pit, "Requested 14:20") with the holder's **real granted capabilities** (`src/lib/mobile/site-capabilities.ts`, resolved via `can(session, …)`). **Cleared-only, never a denied list** — many base field actions are RLS-gated, not capability-gated, so absence ≠ "can't". The physical-badge list is retitled **Credentials**; capabilities own **Access & Permissions**.
- **#5 Task-detail timer (built):** per-task Start / Stop & Log on the task detail (`TaskTimer`). Migration `20260720194801_time_entries_task_link` adds `time_entries.task_id`. Task-timer entries use `activity_category='task'`, which is **isolated from the shift clock** (the clock's open-entry read excludes it, so a running timer never reads as "clocked in") **and from payroll** (`20260720195621_compile_timesheets_exclude_task_timer` excludes `'task'` so it can't double-count against the shift-of-record). Notes/Documents were NOT re-added — they already exist under other names, and re-adding would fight kit-31's deliberate task-detail redesign.
- **Deliberately NOT changed:** systemic Title-Case (repo canon per **D2**, guarded); real-data reimplementations flagged by auditors (catalog lookup, advance cart→single-item, scan modes, approvals taxonomy, inventory status→cost) — all ratified kit-31/32/34 evolution with real backing.
- **Not excluded / correctly omitted:** Settings gap #4 (Sounds/Quiet Hours/Passcode & Biometrics) stays out — re-adding unbacked native-bridge toggles would be fake affordances.

Validation: tsc 0, eslint 0, full vitest **1772/1772**. Three prod migrations applied via Supabase MCP (all additive/backward-compatible). Not yet pushed; a full production build + an authed `/m` browser smoke of the clock/pass/task surfaces are the remaining pre-push steps.
