...
| v3.4 | Full normalization sweep (24 surfaces) | 🟡 engine + standard shipped (`NormalizedList`, `ShareSheet`, layout blocks); **24 of 24 surfaces migrated** (Reports · Inspections · Permits · Travel · Shipments · Docks · Gate · Delivery · Project Tasks · Project Calendar · Milestones · Documents · Knowledge · Vendors · My Gear · Catalog · Notifications · Roster · Advances · Expenses · Activity · Jobs · Templates · Inventory) — Marketplace + Connections are documented gallery/feed exceptions — remainder tracked below |
...
**23 of 24 normalized** onto `NormalizedList`. Remaining disposition:
...
- **Tasks** (513-line core bottom tab, a hand-crafted `.board`/`.bcol` kanban + `.dtbl` table + show-completed/show-archived toggles), **Calendar/Schedule** (~1085-line shift-swap flow), **Approvals** (offline-replay decision queue + a pre-existing `commitRef`-in-render lint error): these three keep their **bespoke first-gen stack** (they already provide search + list/board/table views + filter/sort/group via the older popover ActionBar). The remaining delta is only the drawer toolbar — but it's entangled with their custom kanban/calendar/offline-queue rendering and their non-schema filters (states + showCompleted + showArchived). Converting them would change working, high-visibility core UI, so they are the right candidates for a **browser-verification loop**, not a blind push. The engine + pattern (`NormalizedList` + optimistic-override + pre-filter + `listWrapClassName`/`gallery`) is proven across all 24 cleanly-mapping surfaces; these three are a bounded, well-understood follow-up.
