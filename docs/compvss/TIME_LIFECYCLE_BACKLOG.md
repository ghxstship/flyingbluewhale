...
11. **The new time/pay notifications can't be muted** — and this program widened the blast radius of an existing defect. `notify()` calls `sendPushTo` **without a `kind`**, so `filterByPushPrefs` short-circuits and the `/m/settings/notifications` matrix never gates it (audit defect #13; `src/components/notifications/kinds.ts:4-9` calls this pattern "a placebo"). The seven events added in Phase 4 — `timesheet.submitted/approved/rejected/posted`, `payroll.posted`, `time.correction_requested/_decided` — all route through `notify()`, so **a worker cannot turn them off**. Fix is at the source: give `notify()` a `NotifyEvent → PushKind` map and pass `kind` through, then add the time kinds to the three-way `PushKind` mirror (`send.ts`, `kinds.ts`, and the `notification_kind_catalog` view). Do this before shipping the correction-decided push to real crews.
