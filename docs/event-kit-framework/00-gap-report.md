...
| R11 | **Domain-lib pattern**                                                         | `src/lib/marketplace.ts`, `src/lib/connecteam.ts` (enum tuples `as const` + derived types + helpers)                                                                                                                                            | Precedent shape for a new `src/lib/eventkit/`                                                                                                                                                                                                                                                        | **Follow**                                                                                  |
...
- `src/lib/eventkit/index.ts` — enum tuples (`KIT_SCALES = ['S','M','L','XL']`, sense list, option types) + types derived `as const`, mirroring `marketplace.ts`/`connecteam.ts`.
