...
- [`src/lib/eventkit/index.ts`](../../src/lib/eventkit/index.ts) — canonical axis vocabularies (`KIT_SCALES`, `KIT_PHASES`, `KIT_DISCIPLINES`, `KIT_TIERS`, `KIT_XYZ`, `KIT_LINE_TYPES`, `SENSES`, `OPTION_TYPES`), budget-band↔scale mapping, and the rollup/reconcile/validate helpers (mirrors `marketplace.ts`/`connecteam.ts`).
