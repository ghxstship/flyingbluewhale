...
- `/console/safety/incidents` (+ `/[incidentId]`) — Unified cross-domain incident feed (last 30 days). **CRUD/interactive:** MetricCards, drill-into-domain tiles (ops log / cyber IR / medical), DataTable (filter/group on severity + incident_state, rowHref to detail), "Report incident" link to ops new-incident form; `/[incidentId]` incident detail view.
...
- `/console/email-inbox` (+ `/[id]`) — Inbound project-email register (`inbound_email_messages`, captured at `{slug}@in.atlvs.pro`). **CRUD/interactive:** MetricCards (unrouted/promoted/total), DataTable (filter/group on from + project + routed-to, rowHref to detail); `/[id]` message detail with promote-to-RFI/submittal/transmittal/note action. *(Detail page now exists full-stack — the prior gap is closed.)*
