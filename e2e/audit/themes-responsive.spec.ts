...
// can record a soft "no session" result instead of failing the whole spec
// when the seeded fixture isn't reachable.
...
const ROUTE_FILTER = process.env.AUDIT_ROUTES?.split(",")
  .map((s) => s.trim())
  .filter(Boolean);
          ...
          try {
            ...
            if (realOverflow) {
              logRow({
                ...cell,
                result: "FAIL",
                reason: `horizontal overflow scrollWidth=${overflow.scrollWidth} client=${overflow.clientWidth} vp=${overflow.innerWidth} scrollable=${overflow.scrollable}`,
              });
            }
            ...
            // 5. screenshot
            const shotPath = join(EVIDENCE_ROOT, browserName, bp.name, theme, `${route.name}.png`);
            ensureDir(shotPath);
