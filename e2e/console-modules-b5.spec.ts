...
test.describe("console modules — create flows (batch 5)", () => {
  ...
  test("Inspections · template create", async ({ page }) => {
    const s = stamp();
    await createInModule(page, "/console/inspections/templates/new", {
      code: `INS-${s.slice(-5)}`,
      name: `E2E Template ${s}`,
    });
  });
  ...
  test("Procurement · prequalification questionnaire create", async ({ page }) => {
    const s = stamp();
    await createInModule(page, "/console/procurement/prequalification/questionnaires/new", {
      code: `PQ-${s.slice(-5)}`,
  ...
  test("Legal · insurance policy create", async ({ page }) => {
    const s = stamp();
    await createInModule(page, "/console/legal/insurance/new", {
  ...
  test("Legal · DSAR create", async ({ page }) => {
    await createInModule(page, "/console/legal/privacy/dsar/new", { requester_email: `dsar-${stamp()}@test.example` });
  });

  test("People · credential create", async ({ page }) => {
    await createInModule(page, "/console/people/credentials/new", { number: `CRED-${stamp().slice(-6)}` });
  });

  test("Programs · readiness exercise create", async ({ page }) => {
    await createInModule(page, "/console/programs/readiness/new", { name: `E2E Readiness ${stamp()}` });
  });

  test("Safety · environmental reading create", async ({ page }) => {
    await createInModule(page, "/console/safety/environmental/new", {});
  });

  test("Safety · guard tour create", async ({ page }) => {
    await createInModule(page, "/console/safety/guard-tours/new", { name: `E2E Guard Tour ${stamp()}` });
  });

  test("Safety · major incident create", async ({ page }) => {
    await createInModule(page, "/console/safety/major-incident/new", { name: `E2E Major Incident ${stamp()}` });
  });

  test("Safety · medical encounter create", async ({ page }) => {
    const s = stamp();
    await createInModule(page, "/console/safety/medical/encounters/new", {
  ...
  test("Safety · playbook create", async ({ page }) => {
    const s = stamp();
    await createInModule(page, "/console/safety/playbooks/new", { slug: `e2e-pb-${s}`, title: `E2E Playbook ${s}` });
  });

  test("Safety · safeguarding case create", async ({ page }) => {
    const s = stamp();
    await createInModule(page, "/console/safety/safeguarding/new", {
  ...
  test("Schedule · baseline create", async ({ page }) => {
    await createInModule(page, "/console/schedule/baselines/new", { name: `E2E Baseline ${stamp()}` });
  });

  test("Services · request create", async ({ page }) => {
    await createInModule(page, "/console/services/requests/new", {
  ...
  test("Settings · ticketing integration create", async ({ page }) => {
    await createInModule(page, "/console/settings/integrations/ticketing/new", { label: `E2E Ticketing ${stamp()}` });
  });
  ...
  test.skip("Settings · time-clock zone create", async ({ page }) => {
    await createInModule(page, "/console/settings/time-clock-zones/new", {
      name: `E2E Zone ${stamp()}`,
      center_lat: "25.79",
      center_lng: "-80.13",
      radius_m: "150",
  ...
  test("Sustainability · carbon entry create", async ({ page }) => {
    await createInModule(page, "/console/sustainability/carbon/new", {});
  });

  test("Transmittals · create", async ({ page }) => {
    await createInModule(page, "/console/transmittals/new", { subject: `E2E Transmittal ${stamp()}` });
  });

  test("Transport · arrivals/departures create", async ({ page }) => {
    await createInModule(page, "/console/transport/ad/new", { flight_ref: `FL-${stamp().slice(-5)}` });
  });

  test("Transport · dispatch create", async ({ page }) => {
    const s = stamp();
    await createInModule(page, "/console/transport/dispatch/new", {
  ...
  test("Venues · create", async ({ page }) => {
    await createInModule(page, "/console/venues/new", { name: `E2E Venue ${stamp()}` });
  });

  test("Workforce · badge create", async ({ page }) => {
    const s = stamp();
    await createInModule(page, "/console/workforce/badges/new", { code: `BDG-${s.slice(-5)}`, name: `E2E Badge ${s}` });
  });

  test("Workforce · contractor create", async ({ page }) => {
    const s = stamp();
    await createInModule(page, "/console/workforce/contractors/new", {
  ...
  test("Workforce · onboarding flow create", async ({ page }) => {
    await createInModule(page, "/console/workforce/onboarding/new", { name: `E2E Onboarding ${stamp()}` });
  });

  test("Workforce · roster create", async ({ page }) => {
    await createInModule(page, "/console/workforce/rosters/new", { name: `E2E Roster ${stamp()}` });
  });
});
