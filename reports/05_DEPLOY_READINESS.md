...
| ---- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P2-A | P2       | `.env.local` in this dev environment only populates Supabase URL + anon key. Stripe / Anthropic / Resend / the remote flag service / Sentry absent — related flows (3rd-party round-trips) BLOCKED from local e2e. **This is expected**: prod secrets live in Vercel env, not the repo. | Populate Vercel production env + preview env before enabling those features for real users. Confirmed deployable **without** them (code paths gracefully no-op via `hasResend`, `hasStripe`, etc.). |
...
| `NEXT_PUBLIC_APP_URL`           | Canonical URL for OAuth redirects, sitemap, OG | Deploy domain (e.g., `https://atlvs.pro`) |
...
| `NEXT_PUBLIC_FLAGS_CLIENT_KEY`           | Feature flags                                       | `FLAG_DEFAULTS` fallback — all features static per defaults                                             |
...
4. **Degraded-mode correctness.** Every optional third-party integration (Stripe, Anthropic, Resend, the remote flag service, Sentry) has a `hasFoo` env-gate that makes the related route no-op cleanly when credentials are absent. Deploy-ready today even if those aren't populated yet.
