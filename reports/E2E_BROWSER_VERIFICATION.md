...
preview_eval → {
  ...
  title:   "Marketplace — Live Production Crew, Talent, RFQs · ATLVS Technologies",
...
preview_eval → fetch(..., { redirect: 'manual' }):
  ...
  /api/v1/health                   → 200 { ok: true, data: { service: "atlvs-technologies", status: "ok", version: "v1" } }
```

**Result:** ✅ All five guards behave per UJV baseline. UJV R-1 (personal shell `requireSession`) still active. API health endpoint returns the rebranded service identifier `atlvs-technologies` (confirms ATLVS rebrand successful).
