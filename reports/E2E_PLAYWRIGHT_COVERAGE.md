...
## Deepening interactive coverage — beyond render (page-inventory pass)

Walking the page inventory past render-200 into interactive/functional validation, by interactive-surface density (pages with `actions.ts`): console 258, mobile 16 (compvss-smoke), personal 10, portal 5, auth 3, marketing 1. Console is the bulk; portal pages are mostly read-only displays (the 5 action families are client-proposal lifecycle + artist-advancing).

- **Console create flows now 95/114** `/new` routes. [`console-modules-b7.spec.ts`](../e2e/console-modules-b7.spec.ts) adds **9** previously-uncovered standalone creates (BIM model, Finance budget, Finance lien-waiver, Operations incident, Operations maintenance schedule, Punch item, Warranty, Workforce deployment, Workforce recognition). 5 documented-skip (need a seeded parent FK, distinct pay-period dates, the WebhookEndpointForm event-type selection, or a site-plan event+composite code). The remaining ~19 `/new` routes hard-require pre-existing parent records / file uploads / composite inputs.
- **Detail-page state transitions** — [`console-transitions-b2.spec.ts`](../e2e/console-transitions-b2.spec.ts) adds the **punch-item lifecycle** (create → land on `/console/punch/[id]` → Start → assert next-stage control), exercising the transition server-action through the UI. Fabrication documented-skip (create redirects to the list, not a stable detail URL). This complements the existing lifecycle coverage in `console-core-flows` (invoice/PO/proposal) + `console-transitions` (catalog/task) + `booking-canon` (posting publish→close, offer, settlement, holds).

**Remaining interactive depth to walk next** (by value): portal client-proposal lifecycle (approvals / revisions / change-orders / lifecycle — externally-facing, currently render-only), personal `/me` marketplace flows (applications / talent EPK editor / availability), and detail-page transitions for the remaining state-machine modules whose create lands on a stable detail URL (subscriptions, services/requests, inspections, finance periods/pay-apps).
