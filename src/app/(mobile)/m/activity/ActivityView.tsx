...
export function ActivityView({ rows, labels }: { rows: ActivityRow[]; labels: Labels }) {
  ...
  const colEvent = t("m.activity.col.event", undefined, "Event");
  const colType = t("m.activity.col.type", undefined, "Type");
  const colWhen = t("m.activity.col.when", undefined, "When");

  const FIELDS: FieldDef<ActivityRow>[] = [
    { id: "title", label: colEvent, type: "text", get: (r) => r.title },
    { id: "type", label: colType, type: "select", options: allTypes, get: (r) => r.type },
    { id: "at", label: colWhen, type: "date", get: (r) => r.at, iso: (r) => (r.at ? r.at.slice(0, 10) : null) },
  ];
  ...
  return (
    <NormalizedList
      ...
      empty={{ cols: [colEvent, colType, colWhen], title: labels.emptyTitle, hint: labels.emptyBody }}
    />
  );
}
