  ...
  return (
    <>
      <ModuleHeader
        ...
        title={e.talent?.act_name ?? "Roster Entry"}
        subtitle={`Signed ${e.signed_at ?? "—"}${isEnded ? ` · ended ${e.ended_at}` : ""}`}
        action={
          <div className="flex items-center gap-2">
            <Badge variant={e.exclusive ? "success" : "muted"}>{e.exclusive ? "exclusive" : "non-exclusive"}</Badge>
            {isEnded && <Badge variant="muted">ended</Badge>}
          </div>
        }
      />
      <div className="page-content max-w-xl space-y-5">
        <FormShell action={updateAgencyArtistAction} submitLabel="Save">
          <input type="hidden" name="agency_artist_id" value={e.id} />
          <Input
            label="Commission (bps; 1000 = 10%)"
            name="commission_bps"
            type="number"
            min={0}
            max={5000}
          ...
          />
          <Input label="Signed date" name="signed_at" type="date" defaultValue={e.signed_at ?? ""} />
