...
export default async function Page() {
  ...
  const [offers, holds, settlements] = await Promise.all([
    supabase.from("talent_offers").select("id, offer_phase, fee_cents").eq("org_id", session.orgId),
  ...
  const offerRows = (offers.data ?? []) as Array<{ offer_phase: string; fee_cents: number }>;
  ...
  const liveDeals = offerRows.filter((r) => ["sent", "countered"].includes(r.offer_phase)).length;
  const accepted = offerRows.filter((r) => ["accepted", "contracted"].includes(r.offer_phase)).length;
