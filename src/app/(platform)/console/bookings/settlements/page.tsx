...
export default async function Page() {
  ...
  const { data } = await supabase
    .from("settlements")
    .select(
      "id, show_date, settlement_phase, gross_box_office_cents, nbor_cents, artist_payout_cents, balance_due_cents, paid_attendance, finalized_at, talent_offer_id",
    )
  ...
  const finalCount = rows.filter((r) => r.settlement_phase === "final").length;
  const draftCount = rows.filter((r) => r.settlement_phase === "draft").length;
      ...
      <div className="page-content space-y-5">
        ...
        <DataTable<Row>
          ...
          columns={[
            ...
            {
              key: "settlement_phase",
              header: "Phase",
              render: (r) => <Badge variant={STATUS_TONE[r.settlement_phase] ?? "muted"}>{r.settlement_phase}</Badge>,
              accessor: (r) => r.settlement_phase,
              filterable: true,
              groupable: true,
            },
          ]}
