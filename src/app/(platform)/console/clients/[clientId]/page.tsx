      ...
      <div className="page-content space-y-6">
        ...
        <section className="surface">
          <div className="flex items-center justify-between border-b border-[var(--border-color)] px-5 py-3">
            ...
            <Link href={`/console/proposals?clientId=${client.id}`} className="text-xs text-[var(--org-primary)]">
              View all →
            </Link>
          </div>
          {proposals.length === 0 ? (
            ...
            <table className="data-table">
              ...
              <tbody>
                {proposals.slice(0, 5).map((p) => (
                  <tr key={p.id}>
                    <td>
                      <Link href={`/console/proposals/${p.id}`}>{p.title}</Link>
                    </td>
              ...
              <tbody>
                {invoices.slice(0, 5).map((i) => (
                  <tr key={i.id}>
                    ...
                    <td>
                      <Link href={`/console/finance/invoices/${i.id}`}>{i.title}</Link>
                    </td>
