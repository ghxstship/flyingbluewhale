...
export default async function Page({ params }: { params: Promise<{ connectorId: string }> }) {
  ...
  const row = await getOrgScoped("integration_connectors", session.orgId, p.connectorId);
  if (!row) notFound();
  const title = (row as Record<string, unknown>)["name"] as string | undefined;
  return (
    <>
      <ModuleHeader eyebrow="Record" title={title ?? p.connectorId}
        action={<Button href="/console/integrations" variant="ghost">Back</Button>} />
