...
export default async function Page({ params }: { params: Promise<{ articleId: string }> }) {
  ...
  return (
    <>
      <ModuleHeader
        eyebrow="Record"
        title={title ?? p.articleId}
        action={
          <div className="flex items-center gap-2">
            <Button href="/console/kb" variant="ghost" size="sm">
              Back
            </Button>
            <Button href={`/console/kb/${p.articleId}/edit`} size="sm">
              Edit
            </Button>
            <DeleteForm
              action={deleteArticle.bind(null, p.articleId)}
              confirm={`Delete article "${title ?? "untitled"}"? This cannot be undone.`}
            />
          </div>
        }
      />
