...
export async function createCollectionAction(_: State, fd: FormData): Promise<State> {
  ...
  revalidatePath("/console/legend/resources/collections");
  revalidatePath("/console/legend/resources");
  redirect(`/console/legend/resources/collections/${data.id}`);
}
...
export async function updateCollectionAction(id: string, _: State, fd: FormData): Promise<State> {
  ...
  if (error) return actionFail(error.message, fd);
  revalidatePath("/console/legend/resources/collections");
  revalidatePath(`/console/legend/resources/collections/${id}`);
  revalidatePath("/console/legend/resources");
  redirect(`/console/legend/resources/collections/${id}`);
}
...
export async function deleteCollectionAction(id: string): Promise<void> {
  ...
  revalidatePath("/console/legend/resources/collections");
  revalidatePath("/console/legend/resources");
  redirect("/console/legend/resources/collections");
}
