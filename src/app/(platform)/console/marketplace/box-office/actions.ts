...
export async function createGuestListAction(_: State, fd: FormData): Promise<State> {
  ...
  revalidatePath("/console/marketplace/box-office");
  redirect(`/console/marketplace/box-office/${(data as { id: string }).id}`);
}
...
export async function addGuestEntryAction(_: State, fd: FormData): Promise<State> {
  ...
  if (error) return actionFail(error.message, fd);
  revalidatePath(`/console/marketplace/box-office/${parsed.data.guest_list_id}`);
  return { ok: true };
}
...
export async function checkInEntryAction(_: State, fd: FormData): Promise<State> {
  ...
  if (listId) revalidatePath(`/console/marketplace/box-office/${listId}`);
  return { ok: true };
}
