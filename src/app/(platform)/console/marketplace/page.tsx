...
export default async function Page() {
  ...
  const [postings, calls, offers, applications] = await Promise.all([
    ...
    supabase.from("talent_offers").select("id, talent_offer_phase").eq("org_id", session.orgId),
  ...
  const offerRows = (offers.data ?? []) as Array<{ talent_offer_phase: string }>;
  ...
  const liveOffers = offerRows.filter((r) => r.talent_offer_phase === "sent" || r.talent_offer_phase === "countered").length;
