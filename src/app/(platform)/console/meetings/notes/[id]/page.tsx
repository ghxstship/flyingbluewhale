      ...
      <ModuleHeader
        ...
        action={
          <div className="flex items-center gap-2">
            <Button href="/console/meetings/notes" variant="ghost" size="sm">
              Back
            </Button>
            <RecordActionButton
        ...
        <div className="surface flex flex-wrap items-center gap-3 p-4 text-xs">
          ...
          {note.meeting?.title && (
            <span className="text-[var(--p-text-2)]">
              Meeting:{" "}
              <a className="underline" href={`/console/meetings/${note.meeting.id}`}>
                {note.meeting.title}
              </a>
            </span>
          )}
            ...
            <ul className="divide-y divide-[var(--p-border)]">
              {items.map((a, i) => (
                <li key={i} className="flex flex-wrap items-center gap-2 py-2 text-sm">
                  ...
                  {a.task_id ? (
                    <a className="text-xs underline" href={`/console/tasks/${a.task_id}`}>
