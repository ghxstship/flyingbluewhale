...
export async function createNote(_: State, fd: FormData): Promise<State> {
  ...
  revalidatePath("/console/meetings/notes");
  redirect(`/console/meetings/notes/${(row as { id: string }).id}`);
}
...
export async function summarizeNote(id: string): Promise<void> {
  ...
  if ("error" in result) {
    ...
    // same button.
    revalidatePath(`/console/meetings/notes/${id}`);
    return;
  }
...
export async function deleteNote(id: string): Promise<void> {
  ...
  await supabase
    ...
    .eq("org_id", session.orgId);
  revalidatePath("/console/meetings/notes");
  redirect("/console/meetings/notes");
}
