...
export async function createReservation(_: State, fd: FormData): Promise<State> {
  ...
  revalidatePath("/console/operations/reservations");
  redirect(`/console/operations/reservations/${(row as { id: string }).id}`);
}
...
export async function transitionReservation(_: State, fd: FormData): Promise<State> {
  ...
  revalidatePath("/console/operations/reservations");
  revalidatePath(`/console/operations/reservations/${parsed.data.reservation_id}`);
  return { ok: true };
}
...
export async function deleteReservation(reservationId: string): Promise<void> {
  ...
  await supabase
    ...
    .eq("org_id", session.orgId);
  revalidatePath("/console/operations/reservations");
  redirect("/console/operations/reservations");
}
