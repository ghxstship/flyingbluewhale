...
import { listOnboardingSteps, type OnboardingStepState } from "@/lib/db/onboarding";
import { formatDate } from "@/lib/i18n/format";

export const dynamic = "force-dynamic";

const STATUS_VARIANT: Record<OnboardingStepState, "muted" | "warning" | "success" | "error"> = {
  pending: "muted",
  in_progress: "warning",
  done: "success",
  waived: "muted",
...
const STATUS_LABEL: Record<OnboardingStepState, string> = {
  pending: "Pending",
...
export default async function LetterOnboardingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const done = steps.filter((s) => s.onboarding_step_state === "done" || s.onboarding_step_state === "waived").length;
  const cpOpen = steps.filter((s) => s.critical_path && s.onboarding_step_state !== "done" && s.onboarding_step_state !== "waived").length;
            ...
            <li key={s.id} className="surface flex items-start gap-3 p-4">
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant={STATUS_VARIANT[s.onboarding_step_state]}>{STATUS_LABEL[s.onboarding_step_state]}</Badge>
