...
export default async function Page({ params }: { params: Promise<{ equipmentId: string }> }) {
  ...
  const name = (item["name"] as string | undefined) ?? "Equipment";
    ...
    <>
      ...
      <div className="page-content">
        <div className="mx-auto flex max-w-md flex-col items-center gap-3 rounded-lg border-2 border-dashed border-[var(--border-color)] bg-white p-6 print:border-0 print:p-0">
          ...
          <img src={dataUrl} alt={`QR code for ${name}`} className="h-72 w-72" />
          <div className="text-center">
            <div className="text-base font-semibold text-black">{name}</div>
            {assetTag && <div className="font-mono text-xs text-black/70">Asset: {assetTag}</div>}
            {serial && <div className="font-mono text-[10px] text-black/60">SN: {serial}</div>}
