      ...
      <div className="surface overflow-auto p-2" style={{ maxHeight: "75vh" }}>
        <svg width={totalW} height={totalH} role="img" aria-label="Gantt chart">
          ...
          <g>
            {monthMarks.map((m) => (
              <g key={m.x}>
                <line x1={m.x} y1={HEADER_H - 12} x2={m.x} y2={HEADER_H + chartH} style={{ stroke: "var(--border-color)" }} strokeWidth={1} />
                <text x={m.x + 4} y={HEADER_H - 16} fontSize="10" style={{ fill: "var(--text-muted)" }}>
                  {m.label}
                </text>
              </g>
            ))}
          ...
          {todayX >= LABEL_W && todayX <= LABEL_W + chartW && (
            <g>
              <line
                ...
                strokeDasharray="3 3"
              />
              <text x={todayX + 4} y={HEADER_H - 2} fontSize="10" style={{ fill: "var(--color-info)" }}>
                today
              </text>
            </g>
          )}
          ...
          {/* Row labels */}
          {rendered.map(({ a, y }) => (
            <text key={`label-${a.id}`} x={4} y={y + ROW_H / 2 + 4} fontSize="11" style={{ fill: "var(--text-primary)" }}>
              <tspan fontFamily="ui-monospace, monospace" style={{ fill: "var(--text-muted)" }}>
                {a.code}
              </tspan>
