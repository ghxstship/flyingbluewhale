...
function requiredRankFor(path: string): number {
  ...
  if (path === "/console/settings" || path === "/console/settings/") return 1;
...
export default async function SettingsLayout({ children }: { children: React.ReactNode }) {
  ...
  if (hasSupabase) {
    ...
    if (path && path.startsWith("/console/settings")) {
  ...
  return (
    <div className="flex min-h-full flex-1 flex-col gap-0 md:flex-row">
      ...
      <div className="min-w-0 flex-1">
        {denied ? <AccessDenied requiredRole={denied} backHref="/console" /> : children}
      </div>
    </div>
  );
}
