    ...
    case "dimension":
      return {
        ...
        length_ft: calibrationInchesPerFoot
          ? Math.hypot(live.current.x - live.start.x, live.current.y - live.start.y) /
            (72 * calibrationInchesPerFoot)
          : null,
      };
  }
}
...
function drawMarkup(ctx: CanvasRenderingContext2D, m: MarkupItem, scale: number) {
  ctx.save();
...
export default function MarkupClient({ siteplanId, pdfUrl, calibrationInchesPerFoot }: Props) {
  ...
  const refetchMarkups = useCallback(async () => {
    ...
    if (!res.ok) return;
    const json = (await res.json()) as { data?: { markups: MarkupItem[]; layers: Layer[] } };
    if (json.data) {
