...
export default async function Page() {
  if (!hasSupabase) {
    return (
      <>
        <ModuleHeader eyebrow="Creative" title="Takeoffs" />
  ...
  return (
    <>
      <ModuleHeader
        eyebrow="Creative"
        title="Takeoffs"
        subtitle={`${rows.length} takeoff${rows.length === 1 ? "" : "s"} · ${rows.reduce((s, r) => s + r.item_count, 0)} measurement${rows.length === 1 ? "" : "s"} on file`}
        action={
      ...
      <div className="page-content space-y-5">
        <div className="metric-grid-3">
          <MetricCard label="Takeoffs" value={fmt.number(rows.length)} accent />
          <MetricCard label="Measurements" value={fmt.number(rows.reduce((s, r) => s + r.item_count, 0))} />
          <MetricCard
            label="Drawings Calibrated"
        ...
        <DataTable<Row>
          ...
          emptyLabel="No takeoffs yet"
          emptyDescription="Quantity takeoffs measure drawings: count points, polyline length, polygon area, volume. Items roll up into the parent takeoff total."
          emptyAction={
            <Button href="/console/takeoffs/new" size="sm">
              + New Takeoff
            </Button>
