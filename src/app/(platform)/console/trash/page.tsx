...
export default async function TrashPage({ searchParams }: { searchParams: Promise<{ type?: string }> }) {
  if (!hasSupabase) {
    return (
      <>
        <ModuleHeader eyebrow="Settings" title="Trash" />
  ...
  const activeLabel = TRASH_TYPES.find((t) => t.table === activeTable)?.label ?? "Records";
      ...
      <>
        ...
        <div className="page-content">
          <EmptyState
            ...
            description="Only owners, admins, and managers can browse and restore deleted records."
          />
        </div>
      </>
    );
  ...
  return (
    <>
      <ModuleHeader
        eyebrow="Settings"
        title="Trash"
        subtitle={`${activeLabel} · ${rows.length} deleted record${rows.length === 1 ? "" : "s"} recoverable`}
      />
      <div className="page-content max-w-5xl space-y-4">
        <nav className="flex flex-wrap gap-1.5" aria-label="Record type">
          {TRASH_TYPES.map((t) => {
            const active = t.table === activeTable;
            return (
              <Link
                key={t.table}
                href={`/console/trash?type=${t.table}`}
