...
export default async function Page() {
  ...
  const [offers, holds, settlements] = await Promise.all([
    supabase.from("talent_offers").select("id, talent_offer_state, fee_cents").eq("org_id", session.orgId),
    supabase.from("availability_slots").select("id, kind, tier").eq("kind", "hold"),
    supabase
      .from("settlements")
      .select("id, settlement_state, gross_box_office_cents, balance_due_cents")
      .eq("org_id", session.orgId),
  ]);

  const offerRows = (offers.data ?? []) as Array<{ talent_offer_state: string; fee_cents: number }>;
  ...
  const liveDeals = offerRows.filter((r) => ["sent", "countered"].includes(r.talent_offer_state)).length;
  const accepted = offerRows.filter((r) => ["accepted", "contracted"].includes(r.talent_offer_state)).length;
  const tier1 = holdRows.filter((r) => r.tier === 1).length;
  const finalized = settlementRows.filter((r) => r.settlement_state === "final").length;
    ...
    <>
      ...
      <div className="page-content space-y-5">
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
          ...
          />
          <Tile
            href="/studio/bookings/holds"
            newHref="/studio/bookings/holds/new"
          ...
          />
          <Tile
            href="/studio/bookings/settlements"
          ...
          />
          <Tile
            href="/studio/settings/integrations/ticketing"
