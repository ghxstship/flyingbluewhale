...
export async function updateHospitalityPackage(id: string, _: State, fd: FormData): Promise<State> {
  ...
  if (!result.ok) {
    return { error: result.reason === "stale" ? STALE_ROW_MESSAGE : actionErrorMessage("not-found.rate-card-item", "Rate Card Item not found.") };
  }
  revalidatePath(`/studio/commercial/hospitality/${id}`);
  revalidatePath("/studio/commercial/hospitality");
  redirect(`/studio/commercial/hospitality/${id}`);
}
...
export async function deleteHospitalityPackage(id: string): Promise<void> {
  ...
  revalidatePath("/studio/commercial/hospitality");
  redirect("/studio/commercial/hospitality");
}
