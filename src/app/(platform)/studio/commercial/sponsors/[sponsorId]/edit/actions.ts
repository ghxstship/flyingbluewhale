...
export async function updateSponsorEntitlement(id: string, _: State, fd: FormData): Promise<State> {
  ...
  if (!result.ok) {
    return { error: result.reason === "stale" ? STALE_ROW_MESSAGE : actionErrorMessage("not-found.sponsor-entitlement", "Sponsor Entitlement not found.") };
  }
  revalidatePath(`/studio/commercial/sponsors/${id}`);
  revalidatePath("/studio/commercial/sponsors");
  redirect(`/studio/commercial/sponsors/${id}`);
}
...
export async function deleteSponsorEntitlement(id: string): Promise<void> {
  ...
  revalidatePath("/studio/commercial/sponsors");
  redirect("/studio/commercial/sponsors");
}
