...
export default async function Page({ params }: { params: Promise<{ formId: string }> }) {
  ...
  const { data } = await supabase
    .from("form_defs")
    .select("id, title, slug, status:form_state, description, schema, created_at, updated_at")
    .eq("id", formId)
      ...
      <ModuleHeader
        ...
        action={
          <div className="flex items-center gap-2">
            ...
            <Button href={`/studio/forms/${form.id}/submissions`} variant="secondary" size="sm">
              {t("console.forms.detail.viewSubmissions", undefined, "Submissions")}
            </Button>
            <Button href={`/studio/forms/${form.id}/edit`} variant="secondary" size="sm">
