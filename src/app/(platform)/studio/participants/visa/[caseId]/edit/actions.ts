...
export async function updateVisaCase(id: string, _: State, fd: FormData): Promise<State> {
  ...
  if (!result.ok) {
    return { error: result.reason === "stale" ? STALE_ROW_MESSAGE : actionErrorMessage("not-found.visa-case", "Visa Case not found.") };
  }
  revalidatePath(`/studio/participants/visa/${id}`);
  revalidatePath("/studio/participants/visa");
  redirect(`/studio/participants/visa/${id}`);
}
...
export async function deleteVisaCase(id: string): Promise<void> {
  ...
  revalidatePath("/studio/participants/visa");
  redirect("/studio/participants/visa");
}
