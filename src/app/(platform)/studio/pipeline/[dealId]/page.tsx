...
export default async function DealDetail({ params }: { params: Promise<{ dealId: string }> }) {
  ...
  const pipelineHref = deal.pipeline ? `/studio/pipeline?pipeline=${deal.pipeline.slug}` : "/studio/pipeline";
  ...
  return (
    <>
      <ModuleHeader
        ...
        breadcrumbs={[
          { label: t("console.pipeline.deal.breadcrumbs.sales", undefined, "Sales"), href: "/studio/leads" },
