...
export async function addOnboardingItem(vendorId: string, _prev: State, fd: FormData): Promise<State> {
  ...
  if (!isManagerPlus(session)) return { error: actionErrorMessage("manager-access-required", "Manager access required") };
  const parsed = AddSchema.safeParse({ label: fd.get("label"), required: fd.get("required") === "on" });
  if (!parsed.success) return { error: actionErrorMessage("enter-a-label-for-the-requirement", "Enter a label for the requirement") };
  if (!(await guardVendor(vendorId, session.orgId))) return { error: actionErrorMessage("not-found.vendor", "Vendor not found") };
  ...
  revalidatePath(`/studio/procurement/vendors/${vendorId}/onboarding`);
  return null;
}
...
export async function setOnboardingItemState(
  ...
  const session = await requireSession();
  if (!isManagerPlus(session)) return { error: actionErrorMessage("manager-access-required", "Manager access required") };
  if (!ITEM_STATES.includes(state)) return { error: actionErrorMessage("invalid.state", "Invalid state") };
...
export async function renameOnboardingItem(vendorId: string, itemId: string, label: string): Promise<State> {
  ...
  if (!trimmed || trimmed.length > 160) return { error: actionErrorMessage("label-must-be-1-160-characters", "Label must be 1-160 characters") };
  const db = (await createClient()) as unknown as LooseSupabase;
  const { error } = await db
    .from("vendor_onboarding_items")
...
export async function seedOnboardingDefaults(vendorId: string): Promise<State> {
  ...
  const { data: existing } = await db
    ...
    .eq("vendor_id", vendorId)
    .limit(1);
  if (Array.isArray(existing) && existing.length > 0) return { error: actionErrorMessage("checklist-already-started", "Checklist already started") };
