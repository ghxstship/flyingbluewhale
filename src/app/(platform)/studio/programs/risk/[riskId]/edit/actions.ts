...
export async function updateRisk(id: string, _: State, fd: FormData): Promise<State> {
  ...
  if (!result.ok) {
    return { error: result.reason === "stale" ? STALE_ROW_MESSAGE : actionErrorMessage("not-found.risk", "Risk not found.") };
  }
  revalidatePath(`/studio/programs/risk/${id}`);
  revalidatePath("/studio/programs/risk");
  redirect(`/studio/programs/risk/${id}`);
}
