...
export async function updateMajorIncident(id: string, _: State, fd: FormData): Promise<State> {
  ...
  if (!result.ok) {
    return { error: result.reason === "stale" ? STALE_ROW_MESSAGE : actionErrorMessage("not-found.major-incident", "Major Incident not found.") };
  }
  revalidatePath(`/studio/safety/major-incident/${id}`);
  revalidatePath("/studio/safety/major-incident");
  redirect(`/studio/safety/major-incident/${id}`);
}
...
export async function deleteMajorIncident(id: string): Promise<void> {
  ...
  revalidatePath("/studio/safety/major-incident");
  redirect("/studio/safety/major-incident");
}
