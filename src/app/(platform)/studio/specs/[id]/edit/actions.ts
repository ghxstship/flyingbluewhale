...
export async function updateSpecSection(_: State, fd: FormData): Promise<State> {
  ...
  if (!updated) {
    const { data: stillThere } = await supabase
      ...
      .is("deleted_at", null)
      .maybeSingle();
    return { error: stillThere ? STALE_ROW_MESSAGE : actionErrorMessage("not-found.spec-section", "Spec section not found.") };
  }

  revalidatePath(`/studio/specs/${id}`);
  revalidatePath("/studio/specs");
  redirect(`/studio/specs/${id}`);
}
