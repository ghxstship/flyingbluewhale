...
export async function updateTask(id: string, _: State, fd: FormData): Promise<State> {
  ...
  if (!result.ok) {
    return { error: result.reason === "stale" ? STALE_ROW_MESSAGE : actionErrorMessage("not-found.task", "Task not found.") };
  }
  revalidatePath(`/studio/tasks/${id}`);
  revalidatePath("/studio/tasks");
  redirect(`/studio/tasks/${id}`);
}
