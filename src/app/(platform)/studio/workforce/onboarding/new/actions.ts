...
export async function createFlowAction(_: State, fd: FormData): Promise<State> {
  ...
  if (!isManagerPlus(session)) return { error: actionErrorMessage("auth.manager-plus.author-onboarding-flows", "Only manager+ can author onboarding flows") };
  ...
  redirect(`/studio/workforce/onboarding/${data.id}`);
}
