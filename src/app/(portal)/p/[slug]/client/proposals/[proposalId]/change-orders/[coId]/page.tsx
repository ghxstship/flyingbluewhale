      ...
      <header className="surface p-6">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-3">
          <div className="rounded border border-[var(--border-color)] p-3">
            <div className="eyebrow text-xs text-[var(--text-muted)]">Cost delta</div>
            <div className="mt-1 font-mono text-lg font-semibold">{formatMoney(co.delta_cents ?? 0)}</div>
          </div>
          <div className="rounded border border-[var(--border-color)] p-3">
            <div className="eyebrow text-xs text-[var(--text-muted)]">Requested</div>
            <div className="mt-1 text-sm">
              {co.requested_label ?? "—"} · {timeAgo(co.created_at)}
            </div>
          </div>
          <div className="rounded border border-[var(--border-color)] p-3">
            <div className="eyebrow text-xs text-[var(--text-muted)]">Decision</div>
            <div className="mt-1 text-sm">{co.decided_at ? `${co.state} · ${timeAgo(co.decided_at)}` : "Pending"}</div>
          </div>
        </div>
      </header>
