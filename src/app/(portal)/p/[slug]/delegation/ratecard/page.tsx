...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data } = await supabase
    ...
    .select("id, catalog, status:order_state, total_cents, currency, notes, delegation:delegation_id(name, code), created_at")
  ...
  return (
    <>
      <ModuleHeader
        eyebrow={t("p.delegation.ratecard.eyebrowLabel", undefined, "Portal · Delegation")}
        title={t("p.delegation.ratecard.title", undefined, "Rate-card Orders")}
        subtitle={
          orders.length === 1
            ? t(
