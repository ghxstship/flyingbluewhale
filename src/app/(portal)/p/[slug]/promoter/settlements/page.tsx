        ...
        <section className="mt-5 grid grid-cols-3 gap-2">
          <div className="surface p-3">
            <div className="font-mono text-2xl font-semibold">
              {formatMoney(totalGross, rows[0]?.currency ?? "USD")}
            </div>
              ...
              rows.map((s) => (
                <tr key={s.id}>
                  ...
                  <td className="font-mono text-xs">
                    {formatMoney(s.gross_box_office_cents, s.currency)}
                  </td>
                  <td className="font-mono text-xs">{fmt.number(s.paid_attendance)}</td>
                  <td className="font-mono text-xs">{fmt.number(s.comp_count)}</td>
                  <td className="font-mono text-xs">
                    {formatMoney(s.artist_payout_cents, s.currency)}
                  </td>
                  <td className="font-mono text-xs">
                    {formatMoney(s.balance_due_cents, s.currency)}
                  </td>
                  <td>
