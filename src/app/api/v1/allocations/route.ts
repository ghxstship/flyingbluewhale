...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data, error: dbError } = await supabase.from('catalog_item_allocations').insert({ item_id: body.item_id, project_id: body.project_id, space_id: body.space_id, quantity: body.quantity || 1, notes: body.notes, allocated_by: user.id }).select().single();
    if (dbError) return NextResponse.json({ error: dbError.message }, { status: 400 });
