...
export async function POST(request: NextRequest) {
  ...
  if (scan_type === 'credential') {
    const { data: order, error } = await sb.from('credential_orders').select(`*`).eq('id', scan_data).single();
    if (error || !order) return NextResponse.json({ error: 'Credential not found' }, { status: 404 });
    await sb.from('credential_check_ins').insert({ credential_order_id: order.id, checked_in_by: user.id, method: 'scan', location: body.location });
    if (order.status === 'issued') await sb.from('credential_orders').update({ status: 'picked_up' }).eq('id', order.id);
    ...
    const { data: allocation, error } = await sb.from('catalog_item_allocations').select(`*`).eq('barcode', scan_data).single();
    ...
    if (action === 'checkout') await sb.from('catalog_item_allocations').update({ state: 'checked_out', checked_out_by: user.id }).eq('id', allocation.id);
    else if (action === 'return') await sb.from('catalog_item_allocations').update({ state: 'returned', checked_in_by: user.id, return_condition: body.condition || 'good', return_notes: body.notes }).eq('id', allocation.id);
