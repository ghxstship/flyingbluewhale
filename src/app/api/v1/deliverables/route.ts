...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data, error: dbError } = await supabase.from('deliverables').insert({ project_id: body.project_id, act_id: body.act_id, type: body.type, title: body.title, data: body.data || {}, deadline: body.deadline, submitted_by: user?.id }).select().single();
    if (dbError) return NextResponse.json({ error: dbError.message }, { status: 400 });
