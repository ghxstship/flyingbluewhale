...
export async function POST(req: NextRequest) {
  // Bulk-import bucket — write (60/min). Each request can parse up to
  // 5MB of CSV + 1000 rows; without the gate a misbehaved client
  // could pin the runtime on parse work.
  const rl = await ratelimit({
    key: keyFromRequest(req, "import:vendors"),
    ...RATE_BUDGETS.write,
  });
