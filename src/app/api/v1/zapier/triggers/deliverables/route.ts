...
export async function GET() {
  return withAuth(async (session) => {
    const denied = assertScope(session, "deliverables:read");
    ...
    const { data, error } = await supabase
      .from("deliverables")
      .select(
        "id, project_id, type, title, fulfillment_state, version, submitted_by, submitted_at, reviewed_at, deadline, created_at, updated_at",
      )
