 ...
 *   `https://...lytehaus.tech` and never concat `NEXT_PUBLIC_APP_URL` with
 *   `/console`/`/p`/`/m`."
 *
 ...
 *   1. The literal string `"https://lytehaus.tech"` only appears in a
...
describe("URL canon", () => {
  it('the literal "https://lytehaus.tech" is only allowed in narrowly allowlisted files', () => {
    const offenders: string[] = [];
    for (const file of ALL) {
      const rel = relative(REPO_ROOT, file);
      if (ALLOW_HTTPS_LITERAL.has(rel)) continue;
      const txt = readFileSync(file, "utf8");
      if (txt.includes('"https://lytehaus.tech"') || txt.includes("'https://lytehaus.tech'")) {
        offenders.push(rel);
      }
    }
    expect(
      offenders,
      `Files contain the literal "https://lytehaus.tech" — use SITE.baseUrl from @/lib/seo instead. Offenders: ${offenders.join(", ")}`,
    ).toEqual([]);
  });
