 ...
 *   `https://...lytehaus.live` and never concat `NEXT_PUBLIC_APP_URL` with
 *   `/console`/`/p`/`/m`."
 *
 ...
 *   1. The literal string `"https://lytehaus.live"` only appears in a
...
describe("URL canon", () => {
  it('the literal "https://lytehaus.live" is only allowed in narrowly allowlisted files', () => {
    const offenders: string[] = [];
    for (const file of ALL) {
      const rel = relative(REPO_ROOT, file);
      if (ALLOW_HTTPS_LITERAL.has(rel)) continue;
      const txt = readFileSync(file, "utf8");
      if (txt.includes('"https://lytehaus.live"') || txt.includes("'https://lytehaus.live'")) {
        offenders.push(rel);
      }
    }
    expect(
