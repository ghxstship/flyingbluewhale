  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="center" className="w-80 p-1">
        <div className="px-2 pt-2 pb-1 text-[10px] font-semibold tracking-wide text-[var(--text-muted)]">ACTIONS</div>
        <ul className="grid gap-0.5">
          {registeredActions.length === 0 && (
            <li className="px-2 py-2 text-xs text-[var(--text-muted)]">No actions registered.</li>
          )}
          {registeredActions.map((a) => (
            <li key={a.type}>
              ...
              >
                ...
                <div className="mt-0.5 font-mono text-[10px] text-[var(--text-muted)]">{a.type}</div>
              </button>
            </li>
          ))}
        </ul>
