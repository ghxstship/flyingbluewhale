    ...
    <div className="surface p-6">
            ...
            <div className="grid gap-3 sm:grid-cols-4">
                ...
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {GSA_TIER_PRESETS.map((preset) => (
                    <button
                      ...
                      title={preset.hint}
                    >
                      {preset.label} {formatMoney(preset.rate * 100, { fractionDigits: 0 })}
                    </button>
                  ))}
                </div>
              </div>
              <div className="sm:col-span-2">
                ...
                <div className="mt-1 rounded border border-[var(--border-color)] bg-[var(--background)] px-3 py-2 font-mono text-sm">
                  {formatMoney(Math.max(0, l.days) * Math.max(0, l.rate) * Math.max(0, l.headcount) * 100, { fractionDigits: 0 })}
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="mt-6 border-t border-[var(--border-color)] pt-4">
        <div className="flex items-center justify-between">
          ...
          <div className="font-mono text-2xl font-semibold">{formatMoney(total * 100, { fractionDigits: 0 })}</div>
        </div>
      </div>
    </div>
  );
