...
import { createKudos, toggleReaction } from "./kudos-actions";

/**
 * Shared kudos surface (ADR-0008 Move 1).
 *
 * Recognition feed + give-kudos form + emoji reactions. Same query +
 * render across COMPVSS (`/m/kudos`) and the portal crew persona
 * (`/p/[slug]/crew/kudos`). Both shells share the
 * the deskless-workforce suite/kudos-actions; callers pass the `revalidatePath` so the
 * action invalidates the right route's cache.
 */
...
export async function KudosSurface({
  variant,
  ...
  const title = titleLabel ?? t("m.kudos.title", undefined, "Kudos");
