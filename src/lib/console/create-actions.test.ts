 ...
 * stay in sync with the route tree so every new /console/<x>/new gets a
...
describe("console create actions (⌘K F-A)", () => {
  ...
  it("every action is a static /console/**/new href (no project-scoped params)", () => {
    for (const a of CONSOLE_CREATE_ACTIONS) {
      expect(a.href).toMatch(/^\/console\/.+\/new$/);
