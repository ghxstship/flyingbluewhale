...
export async function updateProject(orgId: string, projectId: string, patch: Partial<Project>): Promise<Project> {
  ...
  // Generated Update type excludes null for nullable enum columns (DB gen
  // artifact). Use LooseSupabase — the canonical escape hatch — so the typed
  // client's RejectExcessProperties constraint doesn't reject valid payloads.
