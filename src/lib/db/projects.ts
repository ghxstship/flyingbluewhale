...
export async function updateProject(orgId: string, projectId: string, patch: Partial<Project>): Promise<Project> {
  const supabase = await createClient();
  // The generated `Update` type's RejectExcessProperties constraint rejects
  // `null` for nullable enum columns (DB gen artifact). Route through
  // LooseSupabase — the canonical escape hatch — to allow the null values.
