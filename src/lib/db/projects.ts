...
export async function updateProject(orgId: string, projectId: string, patch: Partial<Project>): Promise<Project> {
  const supabase = await createClient();
  // The generated Update type omits `| null` for xpms_phase (DB gen artifact
  // for non-nullable-in-schema enums that the custom Project type marks nullable).
  // The two-step cast preserves the target shape without opening `as any`.
  const { data, error } = await supabase
    .from("projects")
