...
export async function updateProject(orgId: string, projectId: string, patch: Partial<Project>): Promise<Project> {
  const supabase = await createClient();
  // The generated `Update` type omits `null` from nullable enum columns (DB
  // gen artifact); the schema accepts null, so cast via unknown to the
  // canonical update shape to keep `as any` out of production code.
  const { data, error } = await supabase
    .from("projects")
