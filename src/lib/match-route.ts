 ...
 *     end-of-string. Stops `/studio/proposals` from claiming
 *     `/studio/projects`.
 *   - Shell roots (`/`, `/studio`, `/m`, `/me`) are active only on
 *     exact match — otherwise the Overview/Home entry lights on every
 *     page in its shell.
 *   - Hrefs carrying a query string (e.g.
 *     `/studio/commercial/hospitality?audience=guest`) identify a
...
const EXACT_ONLY_ROOTS = new Set(["/", "/studio", "/m", "/me"]);
