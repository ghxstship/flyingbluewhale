...
export const WORKFLOWS: readonly Workflow[] = [
  ...
  // ── Money ───────────────────────────────────────────────────────────
  {
    ...
    state: "shipped",
    roles: ALL,
    provenBy: "e2e/compvss-manifest-proofs.spec.ts (expense.file, green vs prod 2026-07-17)",
    note: "G1 — the flagship gap. BUILT: /m/expenses (self-scoped list, explicit submitter_id — expenses RLS is org-member-readable, so 'everyone's spend' is one missing predicate away and /m/requests shipped exactly that bug) + /m/expenses/new mounting the kit expense spec, which had sat in UNMOUNTED_PHOTO_SPECS: written, complete, reachable from nowhere. This is the FIRST surface in the repo to populate expenses.receipt_path — the column existed and no surface, console included, ever wrote it, so 'photograph the receipt' was a product gap that mobile only made obvious. The storage question is settled by precedent, not by widening RLS: `receipts` is service-only (SERVICE_ONLY_BUCKETS) and every existing receipts upload is service-side (the AP-OCR intake), so this uploads service-side too, keeping an org-prefixed path so a future decision to open the bucket needs no re-layout. Receipt upload precedes the insert, and a failed upload degrades to a warning rather than losing the expense — the amount and date are the perishable part; the receipt is still in their camera roll. PROVEN 2026-07-17: the manifest-proofs spec files an expense with a receipt against the live deployment and asserts the row lands WITHOUT the no-receipt mark (the receipt_path IS NULL render).",
  },
  {
