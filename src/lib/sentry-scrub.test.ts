...
describe("scrubCookieHeader", () => {
  it("redacts Supabase auth tokens while preserving benign cookies", () => {
    const header = "lyt_consent=%7B%22essential%22%3Atrue%7D; sb-abc123-auth-token=raw.jwt.value; theme=dark";
    const out = scrubCookieHeader(header) ?? "";
    expect(out).toContain("lyt_consent=%7B%22essential%22%3Atrue%7D");
